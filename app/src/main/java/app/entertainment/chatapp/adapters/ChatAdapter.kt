...
import app.entertainment.chatapp.databinding.ItemReceivedBinding
...
class ChatAdapter(private val context: Context, private var chatMessages: List<ChatMessage>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    inner class SenderViewHolder(val binding: ItemSentBinding) : RecyclerView.ViewHolder(binding.root)

    inner class ReceiverViewHolder(val binding: ItemReceivedBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == ITEM_SENT)
            SenderViewHolder(ItemSentBinding.inflate(LayoutInflater.from(context), parent, false))
        else
            ReceiverViewHolder(ItemReceivedBinding.inflate(LayoutInflater.from(context), parent, false))
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder.javaClass == SenderViewHolder::class.java) {
            ...
            viewHolder.binding.apply {
                messageReceived.text = currentMessage.messageBody
                timestampMessageReceived.text = convertToHHMM(currentMessage.messageTime!!)
                dateReceived.text = currentMessage.messageDate ?: ""
            }
        }
    }
    ...
    fun updateChat(newChats: List<ChatMessage>) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                diffResults.dispatchUpdatesTo(this@ChatAdapter)
            }
        }
    }
